...
<div class="container" >
    @if ($loi!='')
    <center>
        <div style="background-color: #F6CECE; height: 50px; width: 800px; margin: 20px;">
        <p style='color:red; font-size: 17px; text-align: center; padding-top: 10px'>{{$loi}}</p>
        </div>
    </center>
    @endif
</div>
    <?php 
        ...
        $data_ChiTieu[]=[];
        foreach ($data as $row){
            $data_MaTr[$i] = $row->MaTr;
            $data_TenTr[$i] = $row->TenTr;
    ...
    <div class="container" style="font-size: 16px">
        <div class="row">
            <div class="col-lg-3" style="width: 265px; padding-top: 10px">
                @include('layout.sidebar_left')
            </div>
            <div class="col-lg-9">
                <center>
                <div style='width: 800px'>
                    <p style='font-size: 25px; text-align: center'>Biểu đồ</p>
                    <canvas id="bieudohh"></canvas>
                </div>
                </center>
                <br>
                <div>
                    @if (isset($matruong) && isset($manganh))
                        <p style='font-size: 25px; text-align: center'>{{$chitieu}} trường {{$tentruong}} - Ngành {{$tennganh}} qua các năm</p>
                    ...
                    @endif
                    @if (isset($matruong) && isset($nam))
                    <p style='font-size: 17px; text-align: center'>{{$chitieu}} trường {{$tentruong}} - Năm {{$nam}}</p>
                        <table class="table table-bordered">
                        <thead style="background-color: #006666; color: white">
                            <tr>
                                <th>Mã Ngành</th>
                        ...
                        ?>
                    @endif
                    @if (isset($manganh) && isset($nam))
                        <p style='font-size: 17px; text-align: center'>{{$chitieu}} Ngành {{$tennganh}} - Năm {{$nam}} giữa các trường</p>
                        <table class="table table-bordered">
                        <thead style="background-color: #006666; color: white">
                          <tr>
                            <th>Trường</th>
            ...
            @elseif ($loi='')
                <p style='font-size: 25px; text-align: center'>Dữ liệu đang được cập nhật</p>
            @endif
                <br>
            </div>
        </div>
    ...
    <script>
        var matruong = <?php if (isset($matruong)){echo json_encode($matruong);} else {echo '1';}?>;        
        var manganh = <?php if (isset($manganh)){echo json_encode($manganh);} else {echo '1';}?>;
        var nam = <?php if (isset($nam)){echo json_encode($nam);} else {echo '1';}?>;
        var chitieutk = <?php echo json_encode($chitieu);?>;
        var loaibieudo = <?php echo json_encode($bieudo);?>;

        if (matruong!=1 && manganh!=1){
            var cotx =  <?php echo json_encode($data_Nam) ?>;
        }
        if (matruong!=1 && nam!=1){
            var cotx =  <?php echo json_encode($data_TenNg) ?>;
        }
        if (manganh!=1 && nam!=1){
            var cotx =  <?php echo json_encode($data_TenTr) ?>;
        }
        if (chitieutk== 'ChiTieu'){
                var coty =  <?php echo json_encode($data_ChiTieu); ?>
            }else{
