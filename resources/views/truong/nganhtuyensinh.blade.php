          ...
          <div class='kieuchu'>
            <ul class="nav nav-tabs">
              @foreach ($data as $row)
                  ...
                  <li class="active"><a href="">Ngành tuyển sinh</a></li>
              ...
              <tbody>
                  @foreach ($data2 as $row)
                    <tr>
                      ...
                      <td>{{$row->ChiTieu}}</td>
                      <td>{{($row->HocPhi)/1000000}} triệu/năm</td>
                    </tr>
                  @endforeach
              </tbody>
            </table>
    ...
    <div class="row">
        <div class="col-sm-4">
            @foreach ($data as $row)
                ...
                <p class="tentruong2">{{$row->TenTr}}</p>
                <p class= "kieuchu">Mã Trường: {{$row->MaTr}}</p>
                <p class= "kieuchu">{{$row->HeDaoTao}} - {{$row->LoaiTr}}</p >
                <p class= "kieuchu">Địa chỉ: {{$row->DiaChi}}</p >
                <p class= "kieuchu">Email: {{$row->Email}} </p >
                <p class= "kieuchu">Website: <a href="{{$row->Website}} ">{{$row->Website}} </a></p >
                  <p class= "kieuchu">Trang tuyển sinh: <a href="{{$row->WebTS}}">Tại đây</a></p>
         ...
         <div class="col-sm-8">
            ...
            <div>
                  ...
                  <div class="panel-heading">
                    <p><b>Danh sách các ngành tuyển sinh</b></p>
                  </div>
